...
- **Round-type picker title color.** Titles now render as the card's category color (via `var(--cat-solid)`) on a dark translucent plate with a tinted underline and glow shadow — replaces the white-on-colored flat header. Matches the gem accent colors so each card reads as a coherent unit.
