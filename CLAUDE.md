...
**Step 4c — Module state cleanup at the orchestration boundary (2026-04-29).** The DOM cleanup contract (Step 4) had a quiet sibling problem: every module also has a state object (`scribbleState`, `commonThreadState`, `gridLockState`, `nicState`) with an `.active` flag that gates the rest of the codebase. `updateRoleUI` (feud.html:17108-17116) early-returns into the matching module's role-UI handler whenever `<module>State.active` is true. These flags were only cleared inside the module's own `resetXxxState()` called from `mod.reset()` in `handleModuleComplete` — a path that requires the module's flow to reach completion on every client. On non-host phones, missed snapshots / async races can stall the flow and leave `.active=true` carried into the next round. Then `updateRoleUI` early-returns into the WRONG module's handler, which calls `setInputAreaMode('disabled')` (or 'guess') and clobbers the new module's input-area: stepper hidden, input disabled, compose-clue forms wiped, and so on. **Symptoms collected before the fix:** scribble→H5 phone input stuck disabled, scribble→CT clue-giver stepper `display:none`, phone-ct-stage retaining `.active` and pushing next module's content downward — all the same root cause from a different angle.

**Fix (shipped):** `resetModuleCanvas` now calls `resetScribbleState({keepUsedWords: true})` and `resetCommonThreadState()` alongside its existing `resetGridLockState()` / `resetNicState()` calls (feud.html:18119-18152). Contract: **between rounds, all module state is reset by the orchestrator, not by each module's own flow.** The next module's `enter()` function explicitly sets its own `.active=true` after these resets; `nonHostLive` checks (the "module already live from a synced state" detection) still work because they're evaluated AFTER `resetModuleCanvas` runs. Mod.reset() can keep doing its own internal cleanup for normal flow, but resetModuleCanvas is the safety net that doesn't depend on the flow completing. **Lesson for new modules:** if you add a new ROUND_MODULES entry with its own `<module>State.active` gate in `updateRoleUI`, also wire the corresponding `reset<Module>State()` into `resetModuleCanvas`. Otherwise the same class of bugs is back.
...
### Gotcha — `cloneNode` + id-stripped clones lose id-scoped CSS

If a module clones DOM that's styled via id selectors (e.g. `#board-wrapper { display: flex }`) and strips the id afterwards (to avoid duplicate-id collisions in the cloned subtree), the id-scoped rules stop applying to the clone. The clone falls back to whatever its class selectors specify. **Symptom:** the H5/PP summary's answer-list marquee never armed for 6/7-row Surveys because `.h5-board-recap > .board-wrapper(clone) > .h5-recap-answers`'s flex chain was broken — the middle layer reverted to `display: block` (id stripped) so `.h5-recap-answers`' `flex: 1 1 auto; min-height: 0` was inert; `.h5-recap-answers` sized to its own content and `track.scrollHeight - ans.clientHeight` was always ~0. **Fix shape:** declare the missing layout properties on a class selector that survives id stripping (`.h5-board-recap .board-wrapper { display: flex; flex-direction: column; flex: 1 1 auto; min-height: 0 }`). **Pattern for new clones:** when cloning DOM into a summary panel, audit every ancestor in the original chain whose styles came from an id rule, and mirror those rules onto a class selector inside the summary. This is the visual analogue of the cross-parent CSS-variable bug the CT module hit during its phone reparent (see "Phone-controller Common Thread" section).
